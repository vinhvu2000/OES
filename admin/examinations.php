...
fwrite($open, "[$now]: admin truy cập tab bài kiểm tra\n");
fclose($open);
?>
<!DOCTYPE html>
<html>
...
<head>
    <title>OES | Quản lí bài kiểm tra</title>
                                        ...
                                        <div role="tabpanel">
                                            <ul class="nav nav-tabs" role="tablist">
                                                <li role="presentation" class="active"><a href="#tab5" role="tab"
                                                        data-toggle="tab">Bài kiểm tra</a></li>
                                                <li role="presentation"><a href="#tab6" role="tab"
                                                        data-toggle="tab">Thêm bài kiểm tra</a></li>
                                            </ul>
     ...
     echo "error: " . $sql2 . "<br>" . mysqli_error($conn);
                                        ...
                                        <tbody>';
                                                            ...
                                                            while ($row = $result->fetch_assoc()) {
                                                                ...
                                                                if ($trangthai == '1') {
                                                                    ...
                                                                    $stl = '<a href="pages/make_ex_in.php?id=' . $row['mabkt'] . '">Hủy kích hoạt</a>';
                                                                } else {
                                                                    $st = '<p class="text-danger">Không hoạt động</p>';
                                                                    $stl = '<a href="pages/make_ex_ac.php?id=' . $row['mabkt'] . '">Kích hoạt</a>';
                                                                }
                                                                print '
										       <tr>
                                                <td>' . $row['tenmh'] . '</td>
                                                <td>' . $row['tenbkt'] . '</td>
                                                <td>' . $row['maltc'] . '</td>
                                                <td>' . $row['diemtoithieu'] . '</td>
                                                <td>' . ($row['cachtinh']=='caonhat'?'Điểm cao nhất':'Điểm trung bình') . '</td>
                                                <td>' . $row['thoigian'] . '</td>
                                                <td>' . $row['solan'] . '</td>
                                                <td>' . $row['deadline'] . '</td>
                                                <td>' . $row['ghichu'] . '</td>
                                                ...
                                                <ul class="dropdown-menu" role="menu">
                                                    <li>' . $stl . '</li>
													<li><a href="edit-exam.php?id=' . $row['mabkt'] . '">Sửa bài kiểm tra</a></li>
													<li><a href="view-questions.php?id=' . $row['mabkt'] . '">Xem câu hỏi</a></li>
													<li><a href="questions.php?id=' . $row['mabkt'] . '">Thêm câu hỏi</a></li>
                                                    <li><a'; ?> onclick = "return
                                                        confirm('Bạn có chắc chắn muốn xóa bài kiểm tra <?php echo $row['tenbkt']; ?>
                                                        không?')" <?php print ' href="pages/drop_ex.php?id=' . $row['mabkt'] . '">Xóa bài kiểm tra</a></li>
                                                </ul>
                                            </div></td>
                                        ...
                                        Không tìm thấy dữ liệu.
                                                ...
                                                <div role="tabpanel" class="tab-pane fade" id="tab6">
                                                    <form action="pages/add_exam.php" method="POST" name="myform">
     ...
     echo "error: " . $sql2 . "<br>" . mysqli_error($conn);
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <input type="text" class="form-control"
                                                                placeholder="VD: Bài kiểm tra giữa kì" name="tenbkt"
                                                                required autocomplete="off">
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="exampleInputEmail1">Thời gian kiểm tra
                                                                (phút)</label>
                                                            <input type="number" class="form-control"
                                                                placeholder="VD: 15" name="thoigian" required
                                                                autocomplete="off">
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="exampleInputEmail1">Số lần làm tối đa (bỏ qua
                                                                nếu không giới hạn)</label>
                                                            <input min="1" type="number" class="form-control"
                                                                placeholder="VD: 1" name="solan" required
                                                                autocomplete="off">
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="exampleInputEmail1">Điểm tối thiểu (%)</label>
                                                            <input value="80" min="0" max="100" type="number"
                                                                class="form-control"
                                                                placeholder="Nhập điểm số tối thiểu" name="diemtoithieu"
                                                                required autocomplete="off">
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="exampleInputEmail1">Tính điểm cao nhất</label>
                                                            <input type="radio" name="cachtinh" value="caonhat" required
                                                                checked>
                                                            <label for="exampleInputEmail1">Tính điểm trung bình</label>
                                                            <input type="radio" name="cachtinh" value="trungbinh"
                                                                required>
                                                        </div>
                                                        <div class="form-group">
                                                            ...
                                                            <input type="datetime-local" class="form-control"
                                                                ...
                                                                placeholder="Chọn hạn cuối làm bài">
                                                        </div>

                                                        <div class="form-group">
                                                            <label for="exampleInputEmail1">Ghi chú</label>
                                                            <textarea style="resize: none;" rows="6"
                                                                class="form-control" placeholder="Viết gì đó"
                                                                name="ghichu" autocomplete="off"></textarea>
                                                        </div>
                                                        <div class="form-group">
                                                            <input type="hidden" value="false" name="check"></input>
                                                        </div>
                                                        <button type="submit" class="btn btn-primary"
                                                            onclick="document.myform.check.value='true';">Thêm</button>
                                                    </form>
                                                </div>
                                            </div>
                                        </div>
